...
def myHeuristic(position, problem, info={}):
    ...
    def get_loc(cur_x, cur_y):
        # 현재 노드가 도착 노드를 원점 기준으로, 상대적으로 어디에 위치해 있는지 알려준다.
        # 1: 1사분면 2: 2사분면 3: 3사분면 4: 4사분면
    ...
    def get_weight(blocked_pos, relative_loc):
        # 현재 노드가 벽으로 가로 막혀 도착 노드로 갈 수 없다면 우선순위를 낮게 주어야 한다.
        up, down, right, left = blocked_pos
        weight = 0.5
        # bigMaze: weight 0.5일 때 효과적, tinyMaze 또는 mediumMaze일 때에는 0.2일 때 효과적
    ...
    # 일반적인 경우 맨해튼 거리를 그대로, 벽으로 둘러 쌓여 있다면 우선순위를 낮게 (즉 weighted 값을 크게) 주어 방문하지 않도록 유도한다.
